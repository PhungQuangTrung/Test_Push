...
var full = [];
        ...
        const slotItems = ["ga", "tom", "bau", "cua", "ca", "nai"];
            const moneyUser = (await Currencies.getData(event.senderID)).money;
                var moneyBet = parseInt(args[1]);
                    ...
                    if (isNaN(moneyBet) || moneyBet <= 0) return api.sendMessage("[💰] => Số tiền đặt cược không được để trống và phải đặt bằng tiền thật đấy nhé, tiền giả tao méc admin bắt chết cụ mày 😼", event.threadID, event.messageID);
                if (moneyBet > moneyUser) return api.sendMessage("[💰] => Hết tiền rồi ku Dùng lệnh /pqtdeptrai hoặc cút về nhà lấy tiền chơi đi em hehe 😼", event.threadID, event.messageID);
            if (moneyBet < 1000) return api.sendMessage("[💰] => Đặt cọc số tiền trên 1.000 đô thì mới được chơi nha con nghiện 😼", event.threadID, event.messageID);
    ...
    for (let i = 0; i < 3; i++) number[i] = slotItems[Math.floor(Math.random() * slotItems.length)];
        var itemm;
            ...
            api.sendMessage({body:"[💰] => Đang xóc đĩa nha mấy con nghiện :)))",attachment: createReadStream(__dirname + "/cache/baucua.gif")},event.threadID,async (error,info) => {
            ...
            if (i == 1) {
                var mon = parseInt(args[1]) + 300;  
                    await Currencies.increaseMoney(event.senderID, mon); console.log("s1")
                        return api.sendMessage({body:`[💰] => Kết Quả : ${full.join("|")}\n[✤] => Aduu hốc hẳn  ${mon} Đô,Vì Có 1 ${icon}!`,attachment: listimg},event.threadID, event.messageID);
            }
            else if (i == 2) {
                var mon = parseInt(args[1]) * 2; 
                    await Currencies.increaseMoney(event.senderID, mon); console.log("s2")
                        return api.sendMessage({body:`[💰] => Kết Quả : ${full.join("|")}\n[✤] => Aduu hốc hẳn ${mon} Đô,Vì Có 2 ${icon}!`,attachment: listimg},event.threadID, event.messageID);
            }
            else if (i == 3) {
                var mon = parseInt(args[1]) * 3; 
                    await Currencies.increaseMoney(event.senderID, mon); console.log('s3')
                        return api.sendMessage({body:`[💰] => Kết Quả : ${full.join("|")}\n[✤] => Aduu hốc hẳn ${mon} Đô,Vì Có 3 ${icon}!`,attachment: listimg},event.threadID, event.messageID);
            }
